...
ggplot(top_n(application_type,6,Number.of.Requests), aes(x = reorder(Applicant.Type,Number.of.Requests), y= Application, fill= Accepted_Not.Accepted)) + geom_bar(type = 'stacked', stat='identity')+ scale_fill_discrete(name = 'Application status', labels = c('Awarded','Not Awarded'))+ labs(title = "Top 3 Applicants for the Grant And their Approvals in 2016", x= 'Applicant Type' , y= 'Applications')
...
treemap(market_short,c('Goods' ,'State'), vSize = 'percent', type = 'index', title= "Goods and Shortage", overlap.labels = 0.5, align.labels = list(c("left", "top"), c("center", "bottom")))
...
plot_usmap(data = totalOrganic , values = "percent_organic") + 
...
ggplot(rbind(NP, WIC, WICcash, SFMNP, SNAP), aes(x = Region, y= type, fill = AverageGoods))+ geom_tile()+ labs(y = 'Types Payments', x = 'Region', fill= 'Variety of Goods')
